...
async def main():
    ...
    async with websockets.connect(private_url) as ws_private, \
        ...
        # place_order(session=session,symbol='BTCUSDT',order_type='Market',side='Buy',qty=0.01)
        # place_order(session=session,symbol='BTCUSDT',order_type='Market',side='Sell',qty=0.011, reduce_only=True)
