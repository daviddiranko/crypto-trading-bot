...
def get_klines(client: Client,
               ...
               column_names: List[str] = [
                   ...
                   'CloseTime', 'QuoteVolume', 'NumberOfTrades',
    ...
    try:
        df = pd.DataFrame(
            client.get_klines(symbol=symbol,
                              ...
                              limit=limit + finished_klines))[:limit].drop(
    ...
    df.iloc[:, date_columns] = df.iloc[:, date_columns].apply(
        pd.to_datetime, unit='ms') + pd.Timedelta(hours=time_shift)
    non_date_columns = [
        x for x in column_names if column_names.index(x) not in date_columns
    ]
    df.loc[:, non_date_columns] = df.loc[:,
                                         non_date_columns].apply(pd.to_numeric)
    df['Symbol'] = symbol

    return df


def get_historical_klines(client: Client, ticker: str, frequency: int,
                          frequency_unit: str, start: int, start_unit: str):
    '''
    ...
    df = pd.DataFrame(
        client.get_historical_klines(ticker,
    ...
    df[[1, 2, 3, 4, 5, 7, 8, 9, 10]] = df[[1, 2, 3, 4, 5, 7, 8, 9,
                                           10]].apply(pd.to_numeric,
                                                      errors='coerce')
    df.columns = [
        'start', 'open', 'high', 'low', 'close', 'volume', 'end', 'turnover',
        'NumberOfTrades', 'ActiveBuyVolume', 'ActiveBuyQuoteVolume'
    ]
    df[['start', 'end']] = df[['start',
                               'end']].apply(lambda x: x.dt.round(freq='s'))
    df = df.drop(
        columns=['NumberOfTrades', 'ActiveBuyVolume', 'ActiveBuyQuoteVolume'])
