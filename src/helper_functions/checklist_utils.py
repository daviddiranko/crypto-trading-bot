...
def get_sideways(entry_body: float, trading_window: float, sideways_factor: float = 2.0, window_size: int=100, chart_height: float = 3.2, chart_width: float = 4.2) -> int:
    '''
    ...
    sideways_count = max(
        int(
            np.floor((entry_body /
                      (trading_window + 0.00001)) * chart_height *
                     sideways_factor * (window_size / chart_width))), 2)
    
    return sideways_count

def get_body(data: pd.DataFrame, column_1: str, column_2: str, start: int, end: int = 0) -> float:
    '''
    ...
    if end>0:
        body = data.iloc[-start:-end][column_1] - data.iloc[-start:-end][column_2]
    else:
        body = data.iloc[-start:][column_1] - data.iloc[-start:][column_2]

    return body

def get_max_spread(data: pd.DataFrame, column_1: str, column_2: str, start: int, end: int = 0) -> float:
    '''
    ...
    if end>0:
        maximum = max(
            data.iloc[-start:-end][column_1].max(),
            data.iloc[-start:-end][column_2].max())
        minimum = min(
            data.iloc[-start:-end][column_1].min(),
            data.iloc[-start:-end][column_2].min())
    else:
        maximum = max(
            data.iloc[-start:][column_1].max(),
            data.iloc[-start:][column_2].max())
        minimum = min(
            data.iloc[-start:][column_1].min(),
...
def get_trend_series(data: pd.DataFrame, entry_bar: pd.Series, ref_column: str = 'close') -> Tuple[pd.Series, pd.Timedelta]:
    '''
...
def get_linear_regression(data: pd.Series, start: int, end: int=0) -> LinearRegression:
    '''
...
def get_smooth(data: pd.Series, max_dist: float, start: int, end: int =0) -> Tuple[bool, float, float]:
    '''
    ...
    lr_results = get_linear_regression(data=data,
                                              start=start,
                                              end=end)
    ...
    outliers_above = reg_line.loc[(reg_line >
                                               upper_limit_sma_8)]
    outliers_below = reg_line.loc[(reg_line <
                                               lower_limit_sma_8)]
...
def get_recent_high_low(data: pd.DataFrame, n: int) -> Tuple[float, float]:
    ...
    if len(last_lows) > 0:
        last_low = data.min(axis=1).loc[data.min(axis=1) == rolling_lows].iloc[-1]
    else:
        last_low = -np.inf

    if len(last_highs) > 0:
        last_high = data.max(axis=1).loc[data.max(axis=1) == rolling_highs].iloc[-1]
    else:
        last_high = np.inf

    return last_low, last_high

def get_abrupt(data: pd.Series, n: int = 5, threshold: float = 2.0) -> Tuple[bool, pd.Series]:
    '''
...
def get_volatile(data: pd.Series, num_samples: int, max_curvature: float, start: int, end: int =0) -> bool:
    '''
    ...
    consecutive_preceding_slopes = (slope_changes.apply(lambda x: not x)).rolling(num_samples).max().shift(1)==0

    # True if sign of slope did not change in "num_samples" after sample
    consecutive_succeeding_slopes = (slope_changes.apply(lambda x: not x)).sort_index(ascending=False).rolling(num_samples).max().sort_index().shift(-1)==0
    ...
    abs_avg_curvature = (abs_curvature+abs_curvature_successor)/2>=max_curvature

    # volatile is if all criteria are true
    if end>0:
        volatile = (slope_changes * consecutive_preceding_slopes * consecutive_succeeding_slopes * abs_avg_curvature).iloc[-start:-end].fillna(0).max()
    else:
        volatile = (slope_changes * consecutive_preceding_slopes * consecutive_succeeding_slopes * abs_avg_curvature).iloc[-start:].fillna(0).max()
    
    return volatile

def get_strong(data: pd.Series, num_samples: int, threshold: float, start: int, end: int =0, up=True) -> bool:
    '''
...
def get_flat(data: pd.Series, num_samples: int, threshold: float, start: int, end: int =0) -> bool:
    '''
    ...
    strong_up = get_strong(data=data, num_samples=num_samples, threshold=abs(threshold), start=start, end=end, up=True)
    strong_down = get_strong(data=data, num_samples=num_samples, threshold=-abs(threshold), start=start, end=end, up=False)
