...
def checklist_model(model: TradingModel):
    ...
    if (model.account.positions[ticker]['size'] >
        ...
        # otherwise if the stop loss is above the traded price, increase stop loss to the traded price minus 0.5 times the entry body
        elif (last_trade['price'] < last_exit_candle['close']) and (
                new_stop_loss_long >
                model.account.positions[ticker]['stop_loss']):
            ...
            # check if open of candle was above trade price and candle was blue candle, then add it as first higher low
            if (min(last_exit_candle['open'], last_exit_candle['close']) >=
                    last_trade['price']) and (last_exit_candle['close'] >
                                              last_exit_candle['open']):
                if len(model.model_storage['exit_long_higher_lows']) == 0:
    ...
    if (model.account.positions[ticker]['size'] >
        ...
        # otherwise if the stop loss is below the traded price, decrease stop loss to the traded price plus 0.5 times the entry body
        elif ((last_trade['price'] > last_exit_candle['close']) and
              (new_stop_loss_short <
               model.account.positions[ticker]['stop_loss'])):
            ...
            # check if open and close candle was below trade price and candle was red, then add it as first lower high
            if ((max(last_exit_candle['open'], last_exit_candle['close']) <=
                 last_trade['price']) and
                (last_exit_candle['open'] > last_exit_candle['close'])):
