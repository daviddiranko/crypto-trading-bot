...
def checklist_model(model: TradingModel):
    ...
    # Set maximum relative difference between highs / lows to count as support_trend_15 or resistance
    ...
    last_trend_15_candle = model.market_data.history[trend_15_topic].iloc[-1]
    last_trend_5_candle = model.market_data.history[trend_5_topic].iloc[-1]
    ...
    topic_trend_topic_15_delta = entry_bar.name - last_trend_15_candle.name
    ...
    topic_trend_topic_5_delta = entry_bar.name - last_trend_5_candle.name
    ...
    trend_series_15 = model.market_data.history[trend_15_topic]['close'].copy()
    trend_series_5 = model.market_data.history[trend_5_topic]['close'].copy()

    # calculate freq_5uency of trend series, the minimum is necessary to prevent distortions during time shifts
    freq_15 = (trend_series_15.index[1:] - trend_series_15.index[:-1]).min()
    freq_5 = (trend_series_5.index[1:] - trend_series_5.index[:-1]).min()

    # if last timestamp of topic and trend topic are not identical, append last timestamp of topic to trend topic
    if topic_trend_topic_15_delta > pd.Timedelta(minutes=1):
        trend_series_15[entry_bar.name] = entry_bar['close']

    # if last timestamp of topic and trend topic are not identical, append last timestamp of topic to trend topic
    if topic_trend_topic_5_delta > pd.Timedelta(minutes=1):
        trend_series_5[entry_bar.name] = entry_bar['close']
    ...
    # calculate the last resistance_trend_15 line as two consecutive highs that are close together and not succeed by a higher high
    resistances_trend_15 = highs_trend_15.loc[(
        highs_trend_15.diff() /
        highs_trend_15).abs() <= support_resistance_diff]
    if not resistances_trend_15.empty:
        highs_over_resistance_trend_15 = highs_trend_15.loc[highs_trend_15 > (
            resistances_trend_15[-1] * (1 + 1.1 * support_resistance_diff))]
    else:
        highs_over_resistance_trend_15 = pd.Series()

    # check if highs_over_resistance_trend_15 is not empty for stability of calculation
    if not highs_over_resistance_trend_15.empty:

        # check if last high that exceeds resistance_trend_15 was prior to establishment of resistance:
        if highs_over_resistance_trend_15.index[
                -1] < resistances_trend_15.index[-1]:
            resistance_trend_15 = resistances_trend_15[-1] * (
                1 + 1.1 * support_resistance_diff)
        else:
            resistance_trend_15 = None

    elif not resistances_trend_15.empty:
        resistance_trend_15 = resistances_trend_15[-1] * (
            1 + 1.1 * support_resistance_diff)
    else:
        resistance_trend_15 = None

    # calculate the last support_trend_15 line as two consecutive lows that are close together and not broken by a lower low
    supports_trend_15 = lows_trend_15.loc[(
        lows_trend_15.diff() / lows_trend_15).abs() <= support_resistance_diff]
    if not supports_trend_15.empty:
        lows_under_support_trend_15 = lows_trend_15.loc[lows_trend_15 < (
            supports_trend_15[-1] * (1 - 1.1 * support_resistance_diff))]
    else:
        lows_under_support_trend_15 = pd.Series()

    # check if lows_under_support_trend_15  is not empty for stability of calculation
    if not lows_under_support_trend_15.empty:

        # check if last low that breaks support_trend_15 was prior to establishment of support:
        if lows_under_support_trend_15.index[-1] < supports_trend_15.index[-1]:
            support_trend_15 = supports_trend_15[-1] * (
                1 - 1.1 * support_resistance_diff)
        else:
            support_trend_15 = None
    elif not supports_trend_15.empty:
        support_trend_15 = supports_trend_15[-1] * (
            1 - 1.1 * support_resistance_diff)
    else:
        support_trend_15 = None
    ...
    high_low_slopes_trend_15 = get_slopes_highs_lows(lows=lows_trend_15,
                                                     highs=highs_trend_15,
                                                     freq=freq_15)

    # calculate slopes of connection lines between highs and lows of trend series
    high_low_slopes_trend_5 = get_slopes_highs_lows(lows=lows_trend_5,
                                                    highs=highs_trend_5,
                                                    freq=freq_5)
    ...
    derivative_change = (sma_8.shift().diff().iloc[-5:] *
                         sma_8.diff().iloc[-5:]) < 0
    large_curvature = sma_8.diff().diff().iloc[-5:].abs()
    abrupt_blue = (derivative_change * (large_curvature > 2)).sum() > 0
    ...
    # the corridor is relative to the size of the entry bar
    ...
    # multiply by 0.5, because lr_dist should set the width of corridor and not distance from a point to the line
    lr_dist = 0.5 * max_sma_8_dist * open_close_1
    ...
    # calculate relative to the size of the entry bar
    dist_max_dev_sma_8 = (max_dev_sma_8 * np.cos(alpha)) / open_close_1
    ...
    if (entry_bar.name > model.model_storage['entry_bar_time']
           ...
           minutes=15) and topic_long_term_topic_delta < pd.Timedelta(
               minutes=5) and topic_trend_topic_5_delta < pd.Timedelta(
                   minutes=5):
        ...
        if len(lows_trend_15) > 1 and not highs_trend_15.empty:
            ...
            if lows_trend_15.iloc[-1] > lows_trend_15.iloc[-2]:
                ...
                if '1.1.1' not in model.model_stats.keys():
                    model.model_stats['1.1.1'] = {}

                model.model_stats['1.1.1'][ts] = {}
                model.model_stats['1.1.1'][ts]['long'] = 1
        ...
        if (len(lows_trend_5) > 1 and
            ...
            if (lows_trend_5.iloc[-1] > lows_trend_5.iloc[-2]) or (
                ...
                if '1.1.1.1' not in model.model_stats.keys():
                    model.model_stats['1.1.1.1'] = {}

                model.model_stats['1.1.1.1'][ts] = {}
                model.model_stats['1.1.1.1'][ts]['long'] = 1

        # # 1.2.1 Higher lows with retracement on 15 min chart: last low must be higher than previous low + retracement_factor * difference between last high and second last low
        # # check if there are at least one high and two lows for stability of calculations
        # # rules+=1
        # if len(lows_trend_15) > 1 and not highs_trend_15.empty:

        #     # higher lows, but retracement of last low of maximum retracement_factor x difference between last high and second last low
        #     if lows_trend_15.iloc[-1] > lows_trend_15.iloc[-2] + abs(highs_trend_15.iloc[-1]-lows_trend_15.iloc[-2])*(1-retracement_factor):
        #         # long_checklist+=1

        #         if '1.2.1' not in model.model_stats.keys():
        #             model.model_stats['1.2.1']={}

        #         model.model_stats['1.2.1'][ts] = {}
        #         model.model_stats['1.2.1'][ts]['long'] = abs(highs_trend_15.iloc[-1]-lows_trend_15.iloc[-1])/abs(highs_trend_15.iloc[-1]-lows_trend_15.iloc[-2])

        # 1.3.1 Breaking of a significant high on 15 min chart
        ...
        if not highs_trend_15.empty:
            ...
            if highs_trend_15.iloc[-1] < entry_bar['close']:
                ...
                if '1.3.1' not in model.model_stats.keys():
                    model.model_stats['1.3.1'] = {}

                model.model_stats['1.3.1'][ts] = {}
                model.model_stats['1.3.1'][ts]['long'] = 1

        # # 1.4.1 Breaking of a significant high (50% of entry bar) on 15 min chart: 50% of the entry bar must be above last high
        ...
        #     if highs_trend_15.iloc[-1] + high_factor * open_close_1 < entry_bar['close']:
        #         # long_checklist += 1

        #         if '1.4.1' not in model.model_stats.keys():
        #             model.model_stats['1.4.1']={}

        #         model.model_stats['1.4.1'][ts] = {}
        #         model.model_stats['1.4.1'][ts]['long'] =  (entry_bar['close']-highs_trend_15.iloc[-1])/open_close_1
        ...
        if len(lows_trend_5) > 1 and not highs_trend_5.empty:
            ...
            if lows_trend_5.iloc[-1] > lows_trend_5.iloc[-2]:
                ...
                if '1.1.2' not in model.model_stats.keys():
                    model.model_stats['1.1.2'] = {}

                model.model_stats['1.1.2'][ts] = {}
                model.model_stats['1.1.2'][ts]['long'] = 1

        # # 1.2.2 Higher lows with retracement on 5 min chart: last low must be higher than previous low + retracement_factor * difference between last high and second last low
        # # check if there are at least one high and two lows for stability of calculations
        # # rules+=1
        # if len(lows_trend_5) > 1 and not highs_trend_5.empty:

        #     # higher lows, but retracement of last low of maximum retracement_factor x difference between last high and second last low
        #     if lows_trend_5.iloc[-1] > lows_trend_5.iloc[-2] + abs(highs_trend_5.iloc[-1]-lows_trend_5.iloc[-2])*(1-retracement_factor):
        #         # long_checklist+=1

        #         if '1.2.2' not in model.model_stats.keys():
        #             model.model_stats['1.2.2']={}

        #         model.model_stats['1.2.2'][ts] = {}
        #         model.model_stats['1.2.2'][ts]['long'] = abs(highs_trend_5.iloc[-1]-lows_trend_5.iloc[-1])/abs(highs_trend_5.iloc[-1]-lows_trend_5.iloc[-2])

        # 1.3.2 Breaking of a significant high on 5 min chart
        # check if there is at least one for stability of calculations
        rules += 1
        if not highs_trend_5.empty:
            ...
            if highs_trend_5.iloc[-1] < entry_bar['close']:
                ...
                if '1.3.2' not in model.model_stats.keys():
                    model.model_stats['1.3.2'] = {}

                model.model_stats['1.3.2'][ts] = {}
                model.model_stats['1.3.2'][ts]['long'] = 1
        ...
        if not highs_trend_5.empty:
            ...
            if highs_trend_5.iloc[-1] + high_factor * open_close_1 < entry_bar[
                ...
                if '1.4.2' not in model.model_stats.keys():
                    model.model_stats['1.4.2'] = {}

                model.model_stats['1.4.2'][ts] = {}
                model.model_stats['1.4.2'][ts]['long'] = (
                    entry_bar['close'] - highs_trend_5.iloc[-1]) / open_close_1

        # # 1.5 breaking of a resistance_trend_15 line:
        # # check if resistance_trend_15 exists for stability of calculation
        # # rules += 1
        # if resistance:
        ...
        # # rules += 1
        # if len(high_low_slopes_trend_5)>1:
        #     if high_low_slopes_trend_5[-1]<0 and high_low_slopes_trend_5[-2]>0:
        #         # long_checklist += 1
        ...
        # # rules += 1
        # if len(high_low_slopes_trend_5)>1:
        #     if abs(high_low_slopes_trend_5[-1]) < abs(high_low_slopes_trend_5[-2]):
        #         # checklist += 1
        ...
        #         model.model_stats['1.8'][ts]['long'] = high_low_slopes_trend_5[-2]
        #         model.model_stats['1.8'][ts]['short'] = high_low_slopes_trend_5[-2]
        ...
        # # 1.10.1.1 Curvature purple: slope of regression in direction of trade
        # rules += 1
        # if (slope_purple/high_low_candle_window)*candle_window > slope_purple_param:
        #     long_checklist += 1

        #     if '1.10.1.1' not in model.model_stats.keys():
        #         model.model_stats['1.10.1.1']={}

        #     model.model_stats['1.10.1.1'][ts] = {}
        #     model.model_stats['1.10.1.1'][ts]['long'] = (slope_purple/high_low_candle_window)*candle_window

        # 1.10.1.2 Curvature purple: slope of regression flat or in direction of trade in absolute values
        # rules += 1
        if slope_purple > -slope_purple_param:
            # long_checklist += 1

            if '1.10.1.2' not in model.model_stats.keys():
                model.model_stats['1.10.1.2'] = {}

            model.model_stats['1.10.1.2'][ts] = {}
            model.model_stats['1.10.1.2'][ts]['long'] = slope_purple
        ...
        if (slope_blue /
            ...
            model.model_stats['1.12.5'][ts]['long'] = (
                slope_blue / high_low_candle_window) * candle_window
        ...
        # # rules += 1
        # if (sma_8.iloc[-sideways_count:].max() - sma_8.iloc[-sideways_count:].min()) <= blue_amplitude * high_low_candle_window:
        #     # checklist += 1
        ...
        #     model.model_stats['1.12.7'][ts]['long'] = (sma_8.iloc[-sideways_count:].max() - sma_8.iloc[-sideways_count:].min())/high_low_candle_window
        #     model.model_stats['1.12.7'][ts]['short'] = (sma_8.iloc[-sideways_count:].max() - sma_8.iloc[-sideways_count:].min())/high_low_candle_window
        ...
        if not abrupt_blue:
            ...
            if '1.12.9' not in model.model_stats.keys():
                model.model_stats['1.12.9'] = {}

            model.model_stats['1.12.9'][ts] = {}
            model.model_stats['1.12.9'][ts]['long'] = large_curvature
            model.model_stats['1.12.9'][ts]['short'] = large_curvature
        ...
        if (atr_3_1 >= ATR_small_bars_25 * entry_bar['close']).sum() == 0:
            ...
            if '2.3.2' not in model.model_stats.keys():
                model.model_stats['2.3.2'] = {}

            model.model_stats['2.3.2'][ts] = {}
            model.model_stats['2.3.2'][ts]['long'] = atr_3_1.max()
            model.model_stats['2.3.2'][ts]['short'] = atr_3_1.max()
        ...
        #     if model.market_data.history[topic].index[-sideways_count] >= highs_trend_15.index[-1]:
        #         # long_checklist += 1
        ...
        if sideways_count_max - sideways_count_min <= surprise_factor * open_close_1:
            ...
            model.model_stats['2.7'][ts]['long'] = (
                sideways_count_max - sideways_count_min) / open_close_1
            model.model_stats['2.7'][ts]['short'] = (
                sideways_count_max - sideways_count_min) / open_close_1
        ...
        if len(highs_trend_15) > 1 and not lows_trend_15.empty:
            ...
            if highs_trend_15.iloc[-1] < highs_trend_15.iloc[-2]:
                ...
                if ts not in model.model_stats['1.1.1'].keys():
                    model.model_stats['1.1.1'][ts] = {}

                model.model_stats['1.1.1'][ts]['short'] = 1
        ...
        if (len(highs_trend_5) > 1 and
            ...
            if (highs_trend_5.iloc[-1] < highs_trend_5.iloc[-2]) or (
                ...
                if ts not in model.model_stats['1.1.1.1'].keys():
                    model.model_stats['1.1.1.1'][ts] = {}

                model.model_stats['1.1.1.1'][ts]['short'] = 1

        # # 1.2.1 Lower highs with retracement on 15 min chart: last high must be lower than previous high - retracement_factor * difference between las low and second last high
        # # check if there are at least two highs and one low for stability of calculations
        # if len(highs_trend_15) > 1 and not lows_trend_15.empty:

        #     # lower highs_trend_15, but retracement of last high of maximum retracement_factor x difference between last low and second last high
        #     if highs_trend_15.iloc[-1] < highs_trend_15.iloc[-2] - abs(lows_trend_15.iloc[-1]-highs_trend_15.iloc[-2])*(1-retracement_factor):
        #         # short_checklist+=1

        #         if '1.2.1' not in model.model_stats.keys():
        #             model.model_stats['1.2.1']={}

        #         if ts not in model.model_stats['1.2.1'].keys():
        #             model.model_stats['1.2.1'][ts] = {}

        #         model.model_stats['1.2.1'][ts]['short'] = abs(lows_trend_15.iloc[-1]-highs_trend_15.iloc[-1])/abs(lows_trend_15.iloc[-1]-highs_trend_15.iloc[-2])

        # 1.3.1 Breaking of a significant low on 15 min chart
        ...
        if not lows_trend_15.empty:
            ...
            if lows_trend_15.iloc[-1] > entry_bar['close']:
                ...
                if ts not in model.model_stats['1.3.1'].keys():
                    model.model_stats['1.3.1'][ts] = {}

                model.model_stats['1.3.1'][ts]['short'] = 1

        # # 1.4.1 Breaking of a significant low (50% of entry bar) on 15 min chart: 50% of the entry bar must be below last low
        # # check if there is at least one low for stability of calculations
        # if not lows_trend_15.empty:

        #     # at least the "high_factor" fraction of the body of the entry bar has to be below the preceding low
        #     if lows_trend_15.iloc[-1] - high_factor * open_close_1 > entry_bar['close']:
        #         # short_checklist += 1

        #         if '1.4.1' not in model.model_stats.keys():
        #             model.model_stats['1.4.1']={}

        #         if ts not in model.model_stats['1.4.1'].keys():
        #             model.model_stats['1.4.1'][ts] = {}

        #         model.model_stats['1.4.1'][ts]['short'] = (lows_trend_15.iloc[-1]-entry_bar['close'])/open_close_1
        ...
        if len(highs_trend_5) > 1 and not lows_trend_5.empty:
            ...
            if highs_trend_5.iloc[-1] < highs_trend_5.iloc[-2]:
                ...
                if ts not in model.model_stats['1.1.2'].keys():
                    model.model_stats['1.1.2'][ts] = {}

                model.model_stats['1.1.2'][ts]['short'] = 1

        # # 1.2.2 Lower highs with retracement on 5 min chart: last high must be lower than previous high - retracement_factor * difference between las low and second last high
        # # check if there are at least two highs and one low for stability of calculations
        # if len(highs_trend_5) > 1 and not lows_trend_5.empty:

        #     # lower highs_trend_15, but retracement of last high of maximum retracement_factor x difference between last low and second last high
        #     if highs_trend_5.iloc[-1] < highs_trend_5.iloc[-2] - abs(lows_trend_5.iloc[-1]-highs_trend_5.iloc[-2])*(1-retracement_factor):
        #         # short_checklist+=1

        #         if '1.2.2' not in model.model_stats.keys():
        #             model.model_stats['1.2.2']={}

        #         if ts not in model.model_stats['1.2.2'].keys():
        #             model.model_stats['1.2.2'][ts] = {}

        #         model.model_stats['1.2.2'][ts]['short'] = abs(lows_trend_5.iloc[-1]-highs_trend_5.iloc[-1])/abs(lows_trend_5.iloc[-1]-highs_trend_5.iloc[-2])

        # 1.3.2 Breaking of a significant low on 5 min chart
        # check if there is at least one low for stability of calculations
        if not lows_trend_5.empty:
            ...
            if lows_trend_5.iloc[-1] > entry_bar['close']:
                ...
                if ts not in model.model_stats['1.3.2'].keys():
                    model.model_stats['1.3.2'][ts] = {}

                model.model_stats['1.3.2'][ts]['short'] = 1
        ...
        if not lows_trend_5.empty:
            ...
            if lows_trend_5.iloc[-1] - high_factor * open_close_1 > entry_bar[
                ...
                if ts not in model.model_stats['1.4.2'].keys():
                    model.model_stats['1.4.2'][ts] = {}

                model.model_stats['1.4.2'][ts]['short'] = (
                    lows_trend_5.iloc[-1] - entry_bar['close']) / open_close_1

        # # 1.5 breaking of a support_trend_15 line:
        # # check if support_trend_15 exists for stability of calculation
        # if support:
        ...
        #     if ts not in model.model_stats['1.10.1.1'].keys():
        #         model.model_stats['1.10.1.1'][ts] = {}

        #     model.model_stats['1.10.1.1'][ts]['short'] = (slope_purple/high_low_candle_window)*candle_window
        ...
        if slope_purple < slope_purple_param:
            ...
            if ts not in model.model_stats['1.10.1.2'].keys():
                model.model_stats['1.10.1.2'][ts] = {}

            model.model_stats['1.10.1.2'][ts]['short'] = slope_purple
        ...
        if (slope_blue /
            ...
            model.model_stats['1.12.5'][ts]['short'] = (
                slope_blue / high_low_candle_window) * candle_window
        ...
        # # 2.6 sideways not prior to price phase
        # if not lows_trend_15.empty:
        #     if model.market_data.history[topic].index[-sideways_count] >= lows_trend_15.index[-1]:
        #         # short_checklist += 1
        ...
        #     export['Last low'] = lows_trend_15.iloc[-1]
        ...
        #     export['Previous 5 highs'] = highs_trend_15.iloc[-5:]
        #     export['Previous 5 lows'] = lows_trend_15.iloc[-5:]
        #     export['Retracement_factor'] = retracement_factor
        #     export['retracement_long'] = lows_trend_15.iloc[-2] + abs(highs_trend_15.iloc[-1]-lows_trend_15.iloc[-2])*(1-retracement_factor)
        #     export['retracement_short'] = highs_trend_15.iloc[-2] - abs(lows_trend_15.iloc[-1]-highs_trend_15.iloc[-2])*(1-retracement_factor)
        #     export['Support'] = support
        #     export['Resistance'] = resistance
        #     export['high_low_slopes_trend_5 (-1)'] = high_low_slopes_trend_5.iloc[-1]
        #     export['high_low_slopes_trend_5 (-2)'] = high_low_slopes_trend_5.iloc[-2]
