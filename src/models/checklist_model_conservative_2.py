...
def checklist_model(model: TradingModel):
    ...
    # maximum absolute slope of sideways, measured as absolute increase of regression line over the sideways as a fraction of the entrybar
    ...
    increasing = (sma_8.diff().diff().iloc[-5:].mean() >
                  0) and (sma_8.diff().iloc[-1] > 0)

    # decreasing: mean of curvature across last 5 candles < 0 AND last slope < 0
    decreasing = (sma_8.diff().diff().iloc[-5:].mean() <
                  0) and (sma_8.diff().iloc[-1] < 0)
    ...
    y_lr_sideways_start = lr_sideways.predict(X_sideways_candles[0].reshape(
        1, -1))
    y_lr_sideways_end = lr_sideways.predict(X_sideways_candles[-1].reshape(
        1, -1))
    ...
    if (model.account.positions[ticker]['size'] >
        ...
        new_stop_loss_long_2 = np.round(
            max(model.model_storage['entry_close_1_long'],
                model.model_storage['entry_open_1_long']) -
            0.25 * model.model_storage['entry_open_close_1_long'], tick_size)
        ...
        # if the last closing price is above the trade price + the trading fee, adjust stop loss to the trade price + the trading fee
                ...
                last_trade['price'] + 0.1, tick_size)) and (np.round(
                    last_trade['price'] + 0.1,
                    tick_size) > model.account.positions[ticker]['stop_loss']):
        ...
        # increase stop loss to the traded price minus 0.25 times the entry body
                ...
                'price'] and (new_stop_loss_long_2 <
                              last_exit_candle['close']) and (
                                  new_stop_loss_long_2 >
        ...
        # increase stop loss to the traded price minus 0.5 times the entry body
                ...
                'price'] and (
                    new_stop_loss_long < last_exit_candle['close']) and (
                        new_stop_loss_long_2 >= last_exit_candle['close']) and (
                            new_stop_loss_long >
                            model.account.positions[ticker]['stop_loss']):
        ...
        elif len(model.model_storage['exit_long_higher_lows']) > 0 and (
                abs(last_exit_candle['open'] - last_exit_candle['close']) >
                0.02 * model.model_storage['entry_open_close_1_long']):

            # if new candle is higher low, add to higher lows list
            if (last_exit_candle['open'] >
                    model.model_storage['exit_long_higher_lows'][-1]) and (
    ...
    if (model.account.positions[ticker]['size'] >
        ...
        new_stop_loss_short_2 = np.round(
            min(model.model_storage['entry_close_1_short'],
                model.model_storage['entry_open_1_short']) +
            0.25 * model.model_storage['entry_open_close_1_short'], tick_size)
        ...
        # increase stop loss to the traded price plus 0.25 times the entry body
                ...
                'price'] and (new_stop_loss_short_2 >
                              last_exit_candle['close']) and (
                                  new_stop_loss_short_2 <
                                  model.account.positions[ticker]['stop_loss']):
        ...
        # increase stop loss to the traded price plus 0.5 times the entry body
                ...
                'price'] and (
                    new_stop_loss_short > last_exit_candle['close']) and (
                        new_stop_loss_short_2 <= last_exit_candle['close']
                    ) and (new_stop_loss_short <
                           model.account.positions[ticker]['stop_loss']):
        ...
        # track higher lows and exit as soon as 2 lows are broken
        # only count candles that have "sizable" body
        elif len(model.model_storage['exit_short_lower_highs']) > 0 and (
                abs(last_exit_candle['open'] - last_exit_candle['close']) >
                0.02 * model.model_storage['entry_open_close_1_short']):

            # if new candle is higher low, add to higher lows list
            if (last_exit_candle['open'] <
                    model.model_storage['exit_short_lower_highs'][-1]) and (
                        last_exit_candle['close'] <
                        model.model_storage['exit_short_lower_highs'][-1]):
    ...
    if (entry_bar.name > model.model_storage['entry_bar_time']
        ...
        if len(high_low_slopes_trend) > 1:
            if abs(high_low_slopes_trend[-1]) < abs(high_low_slopes_trend[-2]):
                ...
                model.model_stats['1.8'][ts]['short'] = high_low_slopes_trend[
                    -2]
        ...
        if slope_purple > -slope_purple_param:
            ...
            model.model_stats['1.10.1'][ts]['long'] = slope_purple
        ...
        if (body_sideway_count_core_1 >= small_bars_50 * high_low_1).sum() == 0:
            ...
            if '2.1' not in model.model_stats.keys():
                model.model_stats['2.1'] = {}

            model.model_stats['2.1'][ts] = {}
            model.model_stats['2.1'][ts][
        ...
        # rules += 1
        if abs(sma_8.iloc[-1] -
               sma_24.iloc[-1]) < max_distance_sync * entry_bar['open']:
            # checklist += 1
            ...
            model.model_stats['2.5.1'][ts][
                'long'] = sma_8.iloc[-1] - sma_24.iloc[-1]
            model.model_stats['2.5.1'][ts][
                'short'] = sma_8.iloc[-1] - sma_24.iloc[-1]

        # 2.5.2 Blue and Entry bar in Sync: Small distance between sma8 and entrybar
        # rules += 1
        if (min(entry_bar['open'], entry_bar['close']) -
                max_distance_sync * entry_bar['open']) < sma_8.iloc[-1] < (
                    max(entry_bar['open'], entry_bar['close']) +
                    max_distance_sync * entry_bar['open']):
            # checklist += 1
            ...
            model.model_stats['2.5.2'][ts]['long'] = [
                (min(entry_bar['open'], entry_bar['close']) -
                 max_distance_sync * entry_bar['open']),
                (max(entry_bar['open'], entry_bar['close']) +
                 max_distance_sync * entry_bar['open'])
            ]
            model.model_stats['2.5.2'][ts]['short'] = [
                (min(entry_bar['open'], entry_bar['close']) -
                 max_distance_sync * entry_bar['open']),
                (max(entry_bar['open'], entry_bar['close']) +
                 max_distance_sync * entry_bar['open'])
        ...
        if not highs_trend.empty:
            ...
            if model.market_data.history[topic].index[
                    -sideways_count] >= highs_trend.index[-1]:
                # long_checklist += 1
                ...
                model.model_stats['2.6'][ts][
                    'long'] = model.market_data.history[topic].index[
                        -sideways_count]
        ...
        if sideways_count_max - sideways_count_min <= surprise_factor * open_close_1:
            ...
            model.model_stats['2.7'][ts]['long'] = [
                sideways_count_max, sideways_count_min
            ]
        ...
        # rules += 1
        if abs(total_lr_sideways_increase) < max_slope_sideways * open_close_1:
            # checklist += 1
            ...
            model.model_stats['2.8.1'][ts]['long'] = total_lr_sideways_increase
            model.model_stats['2.8.1'][ts]['short'] = total_lr_sideways_increase
        ...
        # 2.10 amplitude of blue must not exceed fixed percentage of trading screen
        # rules += 1
        if (sma_8.iloc[-sideways_count:].max() -
            ...
            if '2.10' not in model.model_stats.keys():
                model.model_stats['2.10'] = {}

            model.model_stats['2.10'][ts] = {}
            model.model_stats['2.10'][ts]['long'] = (
                sma_8.iloc[-sideways_count:].max() -
                sma_8.iloc[-sideways_count:].min()) / high_low_candle_window
            model.model_stats['2.10'][ts]['short'] = (
                sma_8.iloc[-sideways_count:].max() -
                sma_8.iloc[-sideways_count:].min()) / high_low_candle_window
        ...
        # rules += 1
        # core_rules += 1
        if (opposite_fifths_2 * high_low_1 >= low_open_1):
            # long_checklist += 1
            # core_long_checklist += 1
        ...
        # rules += 1
        if sideways_count_max < entry_bar[
                'close'] - sideways_channel * open_close_1:
            # long_checklist +=1
            ...
            model.model_stats['3.3.1'][ts]['long'] = (
                entry_bar['close'] - sideways_count_max) / open_close_1
        ...
        if max_reference < entry_bar['close'] - sideways_channel * open_close_1:
            ...
            model.model_stats['3.3.2'][ts]['long'] = (
                entry_bar['close'] - max_reference) / open_close_1
        ...
        if open_close_1 >= surprise_factor_2 * high_low_candle_window:
            ...
            model.model_stats['3.5'][ts][
                'long'] = open_close_1 / high_low_candle_window
            model.model_stats['3.5'][ts][
                'short'] = open_close_1 / high_low_candle_window
        ...
        if slope_purple < slope_purple_param:
            ...
            model.model_stats['1.10.1'][ts]['short'] = slope_purple
        ...
        if not lows_trend.empty:
            if model.market_data.history[topic].index[
                    -sideways_count] >= lows_trend.index[-1]:
                # short_checklist += 1
                ...
                model.model_stats['2.6'][ts][
                    'short'] = model.market_data.history[topic].index[
                        -sideways_count]
        ...
        # 3.1.1 Entry bar in opposite 1/5ths: the wigs of the entry bar must be smaller equal 20% of the candle (High to low) (Non-negotiable)
        if (opposite_fifths_2 * high_low_1 >= high_open_1):
            # short_checklist += 1
            # core_short_checklist += 1
        ...
        # 3.3.1 space between sideways and entry bar: fraction of entry bar that exceeds maximum of sideways
        if sideways_count_min > entry_bar[
                'close'] + sideways_channel * open_close_1:
            # short_checklist +=1
            ...
            model.model_stats['3.3.1'][ts]['short'] = (
                sideways_count_min - entry_bar['close']) / open_close_1
        ...
        if min_reference > entry_bar['close'] + sideways_channel * open_close_1:
            ...
            model.model_stats['3.3.2'][ts]['short'] = (
                min_reference - entry_bar['close']) / open_close_1
        ...
        confidence_score_long = (long_checklist + checklist + 2) / rules
        confidence_score_short = (short_checklist + checklist + 2) / rules
